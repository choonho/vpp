...
uword unformat_ip46_bind (unformat_input_t * input, va_list * args)
{
  ...
  u32 p;
  if ((type != IP46_TYPE_IP6) && unformat(input, "%U:%u", unformat_ip4_address, &ip46->ip4, &p)) {
    if (p > 65535)
      return 0;
    ip46->pad[0] = ip46->pad[1] = ip46->pad[2] = 0;
  } else if ((type != IP46_TYPE_IP4) && unformat(input, "%U:%u", unformat_ip6_address, &ip46->ip6, &p)) {
    if (p > 65535)
      return 0;
  } else {
    return 0;
